...
const PROJECTS = [
  ...
  { id: "002", name: "MathBuddys", subtitle: "Co-Founder - Free K-8 Tutoring Platform", type: "Education", year: "2025-26",
    ...
    url: "https://starrrashes.github.io/MathBuddy/", accent: "#C9A96E" },
...
const DEALS = [
  {
    address: "64 Corncrib Lane",
    location: "Levittown, NY 11756",
    tabs: 17, formulas: "1,273+",
    scenarios: [
      { name: "Cosmetic Flip", purchase: "$625,000", reno: "$250,000", arv: "$850,000", hold: "8 mo", totalCost: "$1,074,025", profit: "-$286,025", roi: "-70.95%", irr: "-84.34%", mao: "$254,575", breakeven: "$1,060,823" },
      { name: "Teardown/Rebuild", purchase: "$625,000", reno: "$1,600,000", arv: "$1,350,000", hold: "19 mo", totalCost: "$2,731,075", profit: "-$1,470,075", roi: "-83.85%", irr: "-68.39%", mao: "-$794,975", breakeven: "$2,769,471" },
      { name: "Negotiated Buy", purchase: "$450,000", reno: "$200,000", arv: "$850,000", hold: "8 mo", totalCost: "$815,850", profit: "-$27,850", roi: "-8.98%", irr: "-13.16%", mao: "$312,450", breakeven: "$790,199" },
    ],
    verdict: "NO",
    insight: "Even at maximum negotiation ($450K), the deal produces negative returns. Break-even requires selling above $790K on a property with $850K ARV -- leaving zero margin for error. Capital better deployed elsewhere.",
    accent: "#D4412B",
  },
  {
    address: "38 Celestial Lane",
    location: "Levittown, NY 11756",
    tabs: 17, formulas: "1,200+",
    scenarios: [
      { name: "Cosmetic Flip", purchase: "$675,000", reno: "$300,000", arv: "$1,050,000", hold: "7 mo", totalCost: "$1,191,525", profit: "-$214,325", roi: "-46.05%", irr: "-65.29%", mao: "$333,575", breakeven: "$1,172,668" },
      { name: "Teardown/Rebuild", purchase: "$675,000", reno: "$825,000", arv: "$1,400,000", hold: "10 mo", totalCost: "$1,836,525", profit: "-$528,225", roi: "-53.34%", irr: "-59.93%", mao: "$28,525", breakeven: "$1,828,957" },
      { name: "Negotiated Buy", purchase: "$450,000", reno: "$200,000", arv: "$850,000", hold: "8 mo", totalCost: "$815,850", profit: "-$27,850", roi: "-8.98%", irr: "-13.16%", mao: "$312,450", breakeven: "$790,199" },
    ],
    verdict: "NO",
    insight: "All three scenarios produce negative after-tax returns. The Levittown new-construction ceiling (~$1.35-1.37M based on closed comps) makes the teardown path unviable. Cosmetic flip margins evaporate after closing, holding, and selling costs.",
    accent: "#E8C547",
  },
];
...
function ProjectPreview({ project, hovered }) {
  ...
  return (
    <div style={{ width: "100%", paddingBottom: "56.25%", borderRadius: 6, position: "relative", overflow: "hidden",
      border: "1px solid " + (hovered ? ac + "40" : "rgba(240,237,230,0.08)"), background: "#0D0D0D",
      transition: "all 0.5s cubic-bezier(0.16,1,0.3,1)", transform: hovered ? "scale(1.01)" : "scale(1)", transformOrigin: "top left" }}>
      <div style={{ position: "absolute", inset: 0, opacity: hovered ? 0.06 : 0.03, transition: "opacity 0.5s",
        backgroundImage: "linear-gradient(" + ac + "20 1px, transparent 1px), linear-gradient(90deg, " + ac + "20 1px, transparent 1px)", backgroundSize: "24px 24px" }} />
      <div style={{ position: "absolute", top: "-30%", right: "-20%", width: "60%", height: "80%",
        background: "radial-gradient(ellipse, " + ac + "15 0%, transparent 70%)", opacity: hovered ? 1 : 0.5, transition: "opacity 0.5s" }} />
      <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 22, background: "rgba(240,237,230,0.03)", borderBottom: "1px solid rgba(240,237,230,0.05)", display: "flex", alignItems: "center", padding: "0 10px", gap: 5 }}>
        ...
        <span style={{ marginLeft: 8, fontFamily: MONO, fontSize: "0.42rem", color: "#4A4744", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{project.url}</span>
      </div>
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, height: 28, background: "rgba(10,10,10,0.6)", borderTop: "1px solid " + ac + "15", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 12px" }}>
        <span style={{ fontFamily: MONO, fontSize: "0.45rem", color: "#5A5751" }}>{project.year}</span>
        <span style={{ fontFamily: MONO, fontSize: "0.45rem", color: ac, opacity: 0.6 }}>LIVE</span>
      </div>
    </div>
  );
}
...
function ProjectCard({ project, index }) {
  ...
  return (
    <div ref={ref} style={{ opacity: isVisible ? 1 : 0, transform: isVisible ? "translateY(0)" : "translateY(50px)", transition: "all 0.8s cubic-bezier(0.16,1,0.3,1) " + (index * 0.06) + "s" }}>
      <a href={project.url} target="_blank" rel="noopener noreferrer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
        ...
        <div className="js-card-grid" style={{ position: "relative", zIndex: 2, display: "grid", gridTemplateColumns: "0.45fr 0.55fr", gap: "2rem", alignItems: "start" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", marginBottom: "1.25rem" }}>
              ...
              <div style={{ width: hovered ? 28 : 14, height: 2, background: project.accent, borderRadius: 1, transition: "width 0.4s" }} />
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div style={{ display: "flex", gap: "0.4rem", flexWrap: "wrap" }}>
              {project.tags.map(tag => <span key={tag} style={{ fontFamily: MONO, fontSize: "0.52rem", letterSpacing: "0.06em", textTransform: "uppercase", color: hovered ? "#8A8680" : "#5A5751", border: "1px solid " + (hovered ? project.accent + "30" : "rgba(240,237,230,0.08)"), padding: "0.3rem 0.65rem", borderRadius: 2, transition: "all 0.3s" }}>{tag}</span>)}
            </div>
          </div>
        </div>
      </a>
...
function DealMetric({ label, value, negative, accent }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.5rem 0", borderBottom: "1px solid rgba(240,237,230,0.04)" }}>
      <span style={{ fontFamily: MONO, fontSize: "0.52rem", letterSpacing: "0.06em", textTransform: "uppercase", color: "#5A5751" }}>{label}</span>
      <span style={{ fontFamily: MONO, fontSize: "0.7rem", fontWeight: 500, color: negative ? "#D4412B" : (accent || "#F0EDE6"), letterSpacing: "0.02em" }}>{value}</span>
    </div>
  );
}
...
function DealCard({ deal, index }) {
  ...
  const sc = deal.scenarios[activeScenario];
  const isNeg = sc.profit.startsWith("-");

  return (
    <div ref={ref} style={{ opacity: vis ? 1 : 0, transform: vis ? "translateY(0)" : "translateY(40px)", transition: "all 0.8s cubic-bezier(0.16,1,0.3,1) " + (index * 0.12) + "s" }}>
      <div style={{ border: "1px solid rgba(240,237,230,0.08)", borderRadius: 8, overflow: "hidden", background: "#0D0D0B" }}>
        {/* Header */}
        <div style={{ padding: "1.5rem 1.75rem 1.25rem", borderBottom: "1px solid rgba(240,237,230,0.06)", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "absolute", top: "-40%", right: "-10%", width: "50%", height: "120%", background: "radial-gradient(ellipse, " + deal.accent + "08 0%, transparent 70%)", pointerEvents: "none" }} />
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", position: "relative" }}>
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "0.5rem" }}>
                <span style={{ fontFamily: MONO, fontSize: "0.5rem", letterSpacing: "0.12em", textTransform: "uppercase", color: deal.accent }}>Investment Analysis</span>
                <span style={{ fontFamily: MONO, fontSize: "0.45rem", color: "#3A3834", padding: "0.15rem 0.5rem", border: "1px solid rgba(240,237,230,0.06)", borderRadius: 2 }}>{deal.tabs} tabs / {deal.formulas} formulas</span>
              </div>
              <h4 style={{ fontFamily: SERIF, fontSize: "clamp(1.4rem, 2.5vw, 1.8rem)", fontWeight: 400, color: "#F0EDE6", margin: 0, letterSpacing: "-0.01em" }}>{deal.address}</h4>
              <p style={{ fontFamily: MONO, fontSize: "0.52rem", color: "#4A4744", margin: "0.2rem 0 0", letterSpacing: "0.04em" }}>{deal.location}</p>
            </div>
            <div style={{ fontFamily: SERIF, fontSize: "1.6rem", fontWeight: 600, color: "#D4412B", lineHeight: 1, padding: "0.4rem 0.8rem", border: "2px solid #D4412B30", borderRadius: 4, background: "#D4412B08" }}>{deal.verdict}</div>
          </div>
        </div>

        {/* Scenario tabs */}
        <div style={{ display: "flex", borderBottom: "1px solid rgba(240,237,230,0.06)" }}>
          {deal.scenarios.map((s, i) => (
            <button key={i} onClick={() => setActiveScenario(i)}
              style={{ flex: 1, padding: "0.7rem 0.5rem", background: i === activeScenario ? "rgba(240,237,230,0.03)" : "transparent", border: "none", borderBottom: i === activeScenario ? ("2px solid " + deal.accent) : "2px solid transparent", cursor: "pointer", transition: "all 0.3s", fontFamily: MONO, fontSize: "0.5rem", letterSpacing: "0.08em", textTransform: "uppercase", color: i === activeScenario ? "#F0EDE6" : "#4A4744" }}>
              {s.name}
            </button>
          ))}
        </div>
        ...
        <div style={{ padding: "1.25rem 1.75rem" }}>
          <div className="js-deal-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0 2rem" }}>
            <div>
              <p style={{ fontFamily: MONO, fontSize: "0.45rem", letterSpacing: "0.12em", textTransform: "uppercase", color: deal.accent, margin: "0 0 0.5rem", paddingBottom: "0.4rem", borderBottom: "1px solid " + deal.accent + "20" }}>The Deal</p>
              <DealMetric label="Purchase" value={sc.purchase} accent={deal.accent} />
              <DealMetric label="Renovation" value={sc.reno} accent={deal.accent} />
              <DealMetric label="Target ARV" value={sc.arv} accent={deal.accent} />
              <DealMetric label="Hold Period" value={sc.hold} accent={deal.accent} />
              <DealMetric label="Total Cost" value={sc.totalCost} accent={deal.accent} />
            </div>
            <div>
              <p style={{ fontFamily: MONO, fontSize: "0.45rem", letterSpacing: "0.12em", textTransform: "uppercase", color: deal.accent, margin: "0 0 0.5rem", paddingBottom: "0.4rem", borderBottom: "1px solid " + deal.accent + "20" }}>Returns</p>
              <DealMetric label="Profit (After Tax)" value={sc.profit} negative={isNeg} />
              <DealMetric label="ROI" value={sc.roi} negative={isNeg} />
              <DealMetric label="IRR (Annualized)" value={sc.irr} negative={isNeg} />
              <DealMetric label="Max Offer (MAO)" value={sc.mao} accent="#6B6860" />
              <DealMetric label="Break-Even Sale" value={sc.breakeven} accent="#6B6860" />
            </div>
          </div>
        </div>

        {/* Insight */}
        <div style={{ padding: "1rem 1.75rem 1.5rem", borderTop: "1px solid rgba(240,237,230,0.04)", background: "rgba(240,237,230,0.015)" }}>
          <p style={{ fontFamily: SANS, fontSize: "0.78rem", fontWeight: 300, lineHeight: 1.7, color: "#6B6860", margin: 0, fontStyle: "italic" }}>
            <span style={{ color: deal.accent, fontStyle: "normal", fontFamily: MONO, fontSize: "0.5rem", letterSpacing: "0.08em", marginRight: "0.5rem" }}>THESIS:</span>
            {deal.insight}
          </p>
        </div>
      </div>
    ...
    <div style={{ background: "#0A0A0A", color: "#F0EDE6", minHeight: "100vh", fontFamily: SANS, WebkitFontSmoothing: "antialiased", overflowX: "hidden" }}>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        @media (max-width: 768px) {
          ...
          .js-card-grid, .js-about-grid, .js-contact-grid, .js-deal-grid { grid-template-columns: 1fr !important; gap: 1.5rem !important; }
          .js-stats-grid { grid-template-columns: repeat(2, 1fr) !important; }
          .js-deals-grid { grid-template-columns: 1fr !important; }
      ...
      {!loaded && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "#0A0A0A", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
          <div style={{ fontFamily: SERIF, fontSize: "clamp(1.5rem, 4vw, 2.5rem)", fontWeight: 300, fontStyle: "italic", color: "#F0EDE6" }}>Joshua Sat</div>
      ...
      <nav style={{ position: "fixed", top: 0, width: "100%", zIndex: 900, padding: scrolled ? "0.9rem 3rem" : "1.5rem 3rem", display: "flex", justifyContent: "space-between", alignItems: "center", background: scrolled ? "rgba(10,10,10,0.92)" : "transparent", borderBottom: scrolled ? "1px solid rgba(240,237,230,0.06)" : "none", transition: "all 0.4s", opacity: loaded ? 1 : 0 }}>
        ...
        <div className="js-nav-desktop" style={{ display: "flex", gap: "2.5rem" }}>
          {["Work", "Analysis", "About", "Contact"].map(l => <a key={l} href={"#" + l.toLowerCase()} style={{ fontFamily: SANS, fontSize: "0.68rem", fontWeight: 400, letterSpacing: "0.12em", textTransform: "uppercase", color: "#6B6860", textDecoration: "none" }}>{l}</a>)}
      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "flex-end", padding: "0 3rem 5rem", position: "relative", overflow: "hidden" }}>
        <HeroTerminalBlock loaded={loaded} />
      ...
      {/* Financial Analysis */}
      <section id="analysis" style={{ maxWidth: 1400, margin: "0 auto", padding: "4rem 3rem 6rem" }}>
        <SectionTitle number="02" label="Financial Analysis" />
        <p style={{ fontFamily: SANS, fontSize: "0.92rem", fontWeight: 300, lineHeight: 1.75, color: "#6B6860", maxWidth: 600, margin: "0 0 2.5rem" }}>
          Full-scope real estate investment models built from scratch in Excel. Each property analyzed across three scenarios with break-even points, NPV, IRR, and maximum allowable offer calculations. Both deals were killed -- the discipline is in walking away.
        </p>
        <div className="js-deals-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
          {DEALS.map((d, i) => <DealCard key={d.address} deal={d} index={i} />)}
        </div>
        <div className="js-stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "2rem", padding: "3rem 0", marginTop: "2rem", borderTop: "1px solid rgba(240,237,230,0.04)" }}>
          <StatItem number="2" label="Properties Analyzed" index={0} accent="#D4412B" />
          <StatItem number="17" label="Tabs Per Model" index={1} />
          <StatItem number="6" label="Scenarios Modeled" index={2} />
          <StatItem number="0" label="Bad Deals Taken" index={3} accent="#C9A96E" />
        </div>
      </section>
...
function HeroTerminalBlock({ loaded }) {
  const [lines, setLines] = useState([]);
  const allLines = useMemo(() => [
    { text: "$ bat --profile", color: "#6B6860" },
    { text: "  sector: finance & esg data", color: "#D4412B" },
    { text: "  NEDN: microplastics api live", color: "#D4412B" },
    { text: "  DECA CAPM: state top-10", color: "#3A5AE8" },
    { text: "  RE model: 17 tabs, 1273 formulas", color: "#E8C547" },
    { text: "  deals killed: 2/2", color: "#C9A96E" },
    { text: "  status: building", color: "#F0EDE6" },
  ], []);
  useEffect(() => {
    if (!loaded) return;
    let i = 0;
    const iv = setInterval(() => { if (i < allLines.length) { setLines(p => [...p, allLines[i]]); i++; } else clearInterval(iv); }, 350);
    return () => clearInterval(iv);
  }, [loaded, allLines]);
  if (!loaded) return null;
  return (
    <div className="hero-terminal-wrapper" style={{ position: "absolute", top: "18%", right: "5%", fontFamily: MONO, fontSize: "0.68rem", lineHeight: 2, zIndex: 1, opacity: 0.55, maxWidth: "clamp(260px, 30vw, 420px)" }}>
      {lines.map((l, i) => <div key={i} style={{ color: l.color, opacity: 0, animation: "termIn 0.3s forwards", animationDelay: (i * 0.04) + "s", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{l.text}</div>)}
      {lines.length < allLines.length && <span style={{ display: "inline-block", width: 6, height: 13, background: "#D4412B", animation: "blink 1s step-end infinite", verticalAlign: "middle" }} />}
    </div>
  );
}
...
function AboutInner() {
  ...
  return (
    <div ref={ref} className="js-about-grid" style={{ maxWidth: 1400, margin: "0 auto", display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "5rem" }}>
      <div>
        <SectionTitle number="03" label="About" inverted />
...
function ContactSection() {
  ...
  return (
    <section id="contact" ref={ref} style={{ maxWidth: 1400, margin: "0 auto", padding: "2rem 3rem 6rem" }}>
      <SectionTitle number="04" label="Contact" />
